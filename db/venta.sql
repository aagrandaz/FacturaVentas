...
CREATE FUNCTION public.articulo_buscar(nom character varying)
...
CREATE FUNCTION public.articulo_mostrar()
    RETURNS TABLE (id integer, codigo character, nombre character varying, descripcion character varying, unidades_stock integer, precio numeric, estatus character)
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.articulo_mostraractivos()
    RETURNS TABLE (id integer, codigo character, nombre character varying, descripcion character varying, unidades_stock integer, precio numeric, estatus character)
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.cliente_buscar(nom character varying)
...
ORDER BY clienteid DESC LIMIT 200;
END;$$;
...
CREATE FUNCTION public.cliente_buscaractivos(nom character varying)
    RETURNS TABLE (id integer, nombre character varying, email character varying, telefono character varying, direccion character varying, codigopostal character varying, ciudad character varying, pais character varying, estatus character)
    LANGUAGE plpgsql
    AS $$BEGIN
RETURN QUERY
...
CREATE FUNCTION public.cliente_mostrar()
    RETURNS TABLE (id integer, nombre character varying, email character varying, telefono character varying, direccion character varying, codigopostal character varying, ciudad character varying, pais character varying, estatus character)
    LANGUAGE plpgsql
    AS $$
BEGIN
...
CREATE FUNCTION public.cliente_mostraractivos()
    RETURNS TABLE (id integer, nombre character varying, email character varying, telefono character varying, direccion character varying, codigopostal character varying, ciudad character varying, pais character varying, estatus character)
    LANGUAGE plpgsql
    AS $$
BEGIN
...
ORDER BY clienteultimatransaccion DESC, clienteid LIMIT 200;
END;$$;
...
CREATE FUNCTION public.factura_mostrar()
...
SELECT s.clienteId                   N,
       ...
       cast(facturaFecha AS DATE) AS Fecha,
       cast(facturaFecha AS TIME) AS Hora,
...
CREATE FUNCTION public.factura_mostrar(numero integer)
    RETURNS TABLE (linea smallint, codigo_articulo character, articulo character varying, descripcion character varying, cantidad integer, precio numeric, descuento numeric, impuesto numeric, neto numeric)
    LANGUAGE plpgsql
    AS $$
BEGIN
RETURN QUERY
...
CREATE FUNCTION public.facturadetalle_insertar(id integer, articuloid integer, precio numeric, cantidad integer,
                                               descuento numeric, impuesto numeric) RETURNS void
    LANGUAGE plpgsql
    AS $$
DECLARE
rprecio NUMERIC;
...
VALUES (id, articuloid, rprecio, cantidad, descuento, impuesto,
...
COMMENT
ON TABLE public.articulohis IS 'Registra las purgaciones hechas en la tabla articulo.';
...
COMMENT
ON TABLE public.articulolog IS 'Registra las inserciones hechas en la tabla articulo.';
...
COMMENT
ON TABLE public.clientehis IS 'Registra las purgaciones hechas en la tabla cliente.';
...
COMMENT
ON TABLE public.clientelog IS 'Registra las inserciones hechas en la tabla cliente.';
...
CREATE TRIGGER "Factura_TOTAL"
    AFTER INSERT
    ON public.facturadetalle
    FOR EACH ROW EXECUTE PROCEDURE public."Calcular_factura_TOTAL"();
...
CREATE TRIGGER "HISTORICO"
    AFTER DELETE
    ON public.cliente
    FOR EACH ROW EXECUTE PROCEDURE public."Cliente_HISTORICO"();
...
CREATE TRIGGER "HISTORICO"
    AFTER DELETE
    ON public.articulo
    FOR EACH ROW EXECUTE PROCEDURE public."Articulo_HISTORICO"();
...
CREATE TRIGGER "LOG"
    AFTER INSERT OR UPDATE ON public.cliente FOR EACH ROW
EXECUTE PROCEDURE public."Cliente_LOG"();
...
CREATE TRIGGER "LOG"
    AFTER INSERT OR UPDATE ON public.articulo FOR EACH ROW
EXECUTE PROCEDURE public."Articulo_LOG"();
...
CREATE TRIGGER "PURGACION"
    AFTER INSERT OR UPDATE ON public.cliente FOR EACH ROW
EXECUTE PROCEDURE public."Cliente_PURGAR"();
...
CREATE TRIGGER "SUMACARGO"
    AFTER INSERT
    ON public.factura
    FOR EACH ROW EXECUTE PROCEDURE public."Sumar_Cargo"();
...
CREATE TRIGGER "ULTIMO_ACCESO"
    AFTER INSERT
    ON public.factura
    FOR EACH ROW EXECUTE PROCEDURE public."Cliente_ACCESO"();
...
ALTER TABLE ONLY public.facturadetalle
    ADD CONSTRAINT fk_facturaid FOREIGN KEY (facturaid) REFERENCES public.factura(facturaid) ON
UPDATE CASCADE
ON
DELETE CASCADE;
...
GRANT
ALL
ON FUNCTION public."Articulo_HISTORICO"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Articulo_LOG"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Calcular_factura_TOTAL"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Cliente_ACCESO"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Cliente_HISTORICO"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Cliente_LOG"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Cliente_PURGAR"() TO usuario;
...
GRANT ALL
ON FUNCTION public."Sumar_Cargo"() TO usuario;
...
GRANT ALL
ON FUNCTION public.articulo_buscar(nom character varying) TO usuario;
...
GRANT ALL
ON FUNCTION public.articulo_mostrar() TO usuario;
...
GRANT ALL
ON FUNCTION public.articulo_mostraractivos() TO usuario;
...
GRANT ALL
ON FUNCTION public.cliente_buscar(nom character varying) TO usuario;
...
GRANT ALL
ON FUNCTION public.cliente_buscaractivos(nom character varying) TO usuario;
...
GRANT ALL
ON FUNCTION public.cliente_mostrar() TO usuario;
...
GRANT ALL
ON FUNCTION public.cliente_mostraractivos() TO usuario;
...
GRANT ALL
ON FUNCTION public.factura_mostrar() TO usuario;
...
GRANT ALL
ON FUNCTION public.factura_mostrar(numero integer) TO usuario;
...
GRANT ALL
ON FUNCTION public.facturadetalle_insertar(id integer, articuloid integer, precio numeric, cantidad integer, descuento numeric, impuesto numeric) TO usuario;
...
GRANT
ALL
ON TABLE public.articulo TO usuario;
...
GRANT ALL
ON TABLE public.articulohis TO usuario;
...
GRANT ALL
ON TABLE public.articulolog TO usuario;
...
GRANT
ALL
ON TABLE public.cliente TO usuario;
...
GRANT ALL
ON TABLE public.clientehis TO usuario;
...
GRANT ALL
ON TABLE public.clientelog TO usuario;
...
GRANT
ALL
ON TABLE public.factura TO usuario;
...
GRANT
ALL
ON TABLE public.facturadetalle TO usuario;
...
ALTER
DEFAULT PRIVILEGES FOR ROLE postgres GRANT
SELECT, USAGE
ON SEQUENCES TO usuario;
...
ALTER
DEFAULT PRIVILEGES FOR ROLE postgres GRANT ALL ON FUNCTIONS  TO usuario;
...
ALTER
DEFAULT PRIVILEGES FOR ROLE postgres GRANT ALL ON TABLES  TO usuario;
